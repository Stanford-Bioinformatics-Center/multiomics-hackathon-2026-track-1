...
out <- edges[, .(entrez_a, symbol_a, entrez_b, symbol_b, combined_score, w_EE, w_RE, w_diff)]
setorder(out, -combined_score, symbol_a, symbol_b)
fwrite(out, file.path(OUT, "03_weighted_edges.csv"))

for (arm in names(Z)) {
  w <- out[[paste0("w_", arm)]]
  message(sprintf("%s: %d edges, median %.2f, range %.1f..%.1f, %d negative",
                  arm, length(w), median(w), min(w), max(w), sum(w < 0)))
}
