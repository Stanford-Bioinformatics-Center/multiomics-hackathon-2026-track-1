#!/usr/bin/env Rscript
# =====================================================================================================
# 15_joint_network_classes.R — STEP 15: THE JOINT NETWORK WITH METABOLITES GROUPED BY CLASS
# =====================================================================================================
#
# PURPOSE (the question this answers)
#   In the step 14 joint network every metabolite is a plain triangle and metabolites of the same kind
#   (organic acids, nucleic acids, fatty acyls, ...) are scattered, so it is hard to see which kinds sit
#   where. This step redraws the joint network with each metabolite class gathered into one named group.
#
# WHAT THIS SCRIPT DOES (plain language)
#   Nothing is recomputed: nodes, edges and weights are read from step 14. The metabolite class is the
#   RefMet super class (step 1c). Proteins stay circles, metabolites triangles.
#   1. A new layout in which metabolites of the same class are pulled together: extra links between every
#      pair of same-class metabolites are added for the layout ONLY (never drawn, never weighted by the data).
#   2. Each class gets a faint outline ("bubble") and its name above it.
#   Figures (as in step 14: "a" = the two arms drawn separately, "b" = their difference):
#     15a_joint_classes_EE_vs_RE.png          endurance layer above resistance layer, one identical layout
#     15b_joint_classes_edge_difference.png   one network whose edges show w_EE - w_RE
#
# HOW TO RUN
#   After step 14:   Rscript network/15_joint_network_classes.R   (about 20 seconds)
#
# DATA AND PROVENANCE
...
#   $HACK_OUT/14_joint_nodes.csv   node, node_type, class, resp_EE, resp_RE, strength_EE, strength_RE
#
# OUTPUTS (files, locations)
#   $HACK_FIG/15a_joint_classes_EE_vs_RE.png, 15b_joint_classes_edge_difference.png (never in the repo)
...
#
# KNOWN LIMITS
#   The grouped layout differs from step 14, so 15a / 15b cannot be overlaid on 14a / 14b; the extra
#   same-class links shape the picture and carry no evidence (grouping says nothing about whether a class
#   behaves alike). Single-metabolite classes get a small outline of their own.
# =====================================================================================================

# Load packages quietly.
suppressMessages({ library(data.table); library(igraph); library(ggplot2); library(ggrepel); library(ggforce) })
...
# Fixed seed so the layout is identical on every run (same as step 14).
SEED <- 20260926
# How many of the strongest proteins to label in each layer (as in step 14; metabolites are named by class).
NLAB <- 10
...
# Nodes with type, class, response and strength.
...
# Show how many metabolites fall in each class.
print(nodes[node_type == "metabolite", .N, by = class][order(-N)])
...
# Helper: labels for the NLAB strongest proteins by a strength column (metabolites are named by class instead).
top_labels <- function(N, col) { N <- copy(N); N[, rk := frank(-get(col), ties.method = "first"), by = node_type]
  N[, lab := fifelse(node_type == "protein" & rk <= NLAB, node, NA_character_)]; N }
# Helper: one class name per class, centred above its members.
class_names <- function(P) P[node_type == "metabolite", .(x = mean(x), y = max(y) + 0.035), by = class]
# Helper: faint class outlines ("bubbles"); expand keeps single metabolites visible.
...
lay <- rbind(grp[, .(node, node_type, class, arm = "EE", x, y = 0.55 + 0.38 * y, resp = resp_EE, strength = strength_EE)],
             grp[, .(node, node_type, class, arm = "RE", x, y = 0.05 + 0.38 * y, resp = resp_RE, strength = strength_RE)])
...
# Protein labels and class names per layer.
lab <- rbindlist(lapply(c("EE", "RE"), function(ar) top_labels(lay[lay$arm == ar], "strength")))
cn <- rbindlist(lapply(c("EE", "RE"), function(ar) class_names(lay[lay$arm == ar])))
...
# Edges at the grouped layout, smallest differences drawn first; colour limit at the 95th percentile of |w_diff|.
D <- seg(grp); lim_d <- as.numeric(quantile(abs(D$w_diff), 0.95)); D <- D[order(abs(w_diff))]
# Per-node absolute strength difference and labels for the proteins with the largest.
N <- copy(grp)[, abs_delta := abs(strength_EE - strength_RE)]; N <- top_labels(N, "abs_delta")
# Draw.
q <- ggplot() +
  # class bubbles
  hulls(N) +
  ...
  geom_point(data = N, aes(x, y, size = abs_delta, shape = node_type), fill = "grey80", colour = "grey25", stroke = 0.22) +
